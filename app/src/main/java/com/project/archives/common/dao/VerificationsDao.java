...
public class VerificationsDao extends AbstractDao<Verifications, byte[]> {
    ...
    @Override
    protected final void attachEntity(Verifications entity) {
    ...
    protected String getSelectDeep() {
        if (selectDeep == null) {
            ...
            builder.append(',');
            SqlUtils.appendColumns(builder, "T0", daoSession.getMultiDictionariesDao().getAllColumns());
            builder.append(" FROM VERIFICATIONS T");
            builder.append(" LEFT JOIN MULTI_DICTIONARIES T0 ON T.\"DISCIPLINE\"=T0.\"ID\"");
            builder.append(' ');
    ...
    protected Verifications loadCurrentDeep(Cursor cursor, boolean lock) {
        Verifications entity = loadCurrent(cursor, 0, lock);
        int offset = getAllColumns().length;

        MultiDictionaries multiDictionaries = loadCurrentOther(daoSession.getMultiDictionariesDao(), cursor, offset);
        entity.setMultiDictionaries(multiDictionaries);

        return entity;    
    }

    public Verifications loadDeep(Long key) {
        assertSinglePk();
        if (key == null) {
            return null;
        }
    ...
    public List<Verifications> loadAllDeepFromCursor(Cursor cursor) {
        int count = cursor.getCount();
        List<Verifications> list = new ArrayList<Verifications>(count);
    ...
    protected List<Verifications> loadDeepAllAndCloseCursor(Cursor cursor) {
        try {
    ...
    public List<Verifications> queryDeep(String where, String... selectionArg) {
